 ...
 * 主入口类是<a href="symbols/BMapLibDra.DrawingManager.html">DrawingManager</a>，
 * 基于Baidu Map API 1.4。
 *
...
/**
 * @namespace BMap的所有library类均放在BMapLibDra命名空间下
 */
var BMapLibDra = window.BMapLibDra = BMapLibDra || {};
...
(function() {
	...
	/**
	 * @exports DrawingManager as BMapLibDra.DrawingManager
	 */
	var DrawingManager =
	/**
	 ...
	 * var myDrawingManagerObject = new BMapLibDra.DrawingManager(map, {isOpen: true,
         ...
         *     });
         */
		BMapLibDra.DrawingManager = function(map, opts){
			if (!map) {
				return;
			}
			instances.push(this);
	...
	DrawingManager.prototype._calculate = function (overlay, point) {
		...
		if (this._enableCalculate && BMapLibDra.GeoUtils) {
			var type = overlay.toString();
			//不同覆盖物调用不同的计算方法
			switch (type) {
				case "[object Polyline]":
					result.data = BMapLibDra.GeoUtils.getPolylineDistance(overlay);
					break;
				case "[object Polygon]":
					result.data = BMapLibDra.GeoUtils.getPolygonArea(overlay);
					break;
				case "[object Circle]":
	...
	DrawingTool.prototype.initialize = function(map){
		...
		container.className = "BMapLibDra_Drawing";
		//用来设置外层边框阴影
		var panel = this.panel = document.createElement("div");
		panel.className = "BMapLibDra_Drawing_panel";
	...
	DrawingTool.prototype._generalHtml = function(map){
		...
		var html = [];
		html.push(getItem("BMapLibDra_box BMapLibDra_hander", "hander"));
		for (var i = 0, len = this.drawingModes.length; i < len; i++) {
			var classStr = 'BMapLibDra_box BMapLibDra_' + this.drawingModes[i];
			if (i == len-1) {
				classStr += ' BMapLibDra_last';
			}
	...
	DrawingTool.prototype.setStyleByDrawingMode = function(drawingType){
		...
		for (var i = 0, len = boxs.length; i < len; i++) {
			...
			if (box.getAttribute('drawingType') == drawingType) {
				var classStr = "BMapLibDra_box BMapLibDra_" + drawingType + "_hover";
				if (i == len - 1) {
					classStr += " BMapLibDra_last";
				}
