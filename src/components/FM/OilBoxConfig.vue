<template>
	<section style="height: 100%" class="row">
		...
		<div class="configPanel">
			...
			<!--多油箱-->
			<div class="multiOilBox" v-show="isShowMultiOilBox">
				<div class="oilBoxItem">
					<div class="oilBox oilBox-first"><span>_油箱_1</span></div>
				...
				<div class="oilBoxItem">
					<div class="oilBox oilBox-first"><span>_油箱_2</span></div>
					<div class="oilBox-first-config">
						<el-row>
							<el-col :span="24">
								<el-col :span="6">_零满值_</el-col>
				...
				<div class="oilBoxItem">
					<div class="oilBox oilBox-first"><span>_油箱_3</span></div>
					<div class="oilBox-first-config">
						<el-row>
							<el-col :span="24">
								<el-col :span="6">_零满值_</el-col>
		...
		methods:{
			...
			handleNodeClick(data){
				if(data.id.indexOf("@A")===-1 && data.id.indexOf("@B")===-1 && data.id.indexOf("@C")===-1){
					...
					$.ajax({
						"url": "/web/FleetManager/FleetManager_System",
						"cache": false,
						"async": true,
						"type": "post",
						"dataType": "json",
						"contentType": "application/json; charset=utf-8",
						"data": '{"FTokenID":"' + _this.$store.state.FTokenID + '","FAction":"QueryFuelConfigByFVehicleGUID","FVehicleGUID":"'+_this.FVehicleGUID+'","FVersion":"1.0.0"}',
			...
			save(type){
				...
				if(_this.FVehicleGUID!=''&&_this.FVehicleGUID!=null){
					...
					if(type=="SingleOilBox"){
						...
						let JsonStr=JSON.stringify({FTokenID:_this.$store.state.FTokenID,FAction:"UpdateFuelCarArgs",FT_FuelCarArgs:JsonArr,FVehicleGUID:_this.FVehicleGUID,FVersion:"1.0.0"});
						...
						$.ajax({
							...
							success: function (json) {
								...
								$.each(_this.OilBoxConfigTreeData[0].children,function (index,obj) {
									$.each(obj.children,function (i,item) {
										if(item.label==_this.FVehicleName){
											if(json.Result==200) {
												item.message = "_保存成功_";
											}else {
												item.message = "_保存失败_";
					...
					if(type=="MultiOilBox"){
						...
						//console.log(JsonArr);
						let JsonStr=JSON.stringify({FTokenID:_this.$store.state.FTokenID,FAction:"AddFuelSensor",FFuelSonserConfigs:JsonArr,FVehicleGUID:_this.FVehicleGUID,FVersion:"1.0.0"});
						$.each(_this.OilBoxConfigTreeData[0].children,function (index,obj) {
							$.each(obj.children,function (i,item) {
								$.each(item.children,function (j,a) {
									if(a.label==_this.FVehicleName){
			...
			deleteConfig(){
				...
				$.ajax({
					...
					"dataType": "json",
					"contentType": "application/json; charset=utf-8",
					"data":'{"FTokenID":"' + _this.$store.state.FTokenID + '","FAction":"DeleteFuelConfig","FVehicleGUIDs":"'+CheckedKeysStr+'","FVersion":"1.0.0"}',
					success:function(json){
						//console.log(json);
						if(json.Result="200"){
							_this.reset();
...
<style scoped>
	...
	/*以下为新加样式*/
	.configPanel .singleOilBox,.configPanel .oilBox-first-config {
		width:500px;
		margin:30px auto;
	}
	...
	.multiOilBox .oilBoxItem .el-col-24,.singleOilBox .el-col-24 {
		margin-top:20px;
	}
