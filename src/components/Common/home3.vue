<template>
  <div>
    ...
    <div class="panel panel-default" v-show="TemperatureAbNormalCount">
      <div class="panel-heading " style="background: #ffffff">
      ...
      <div class="panel-body">
        <div class="carRunStopReport" id="TemperatureAbNormalCount">
        </div>
      </div>
    </div>
...
//          FLineChart6:'',
        ...
        TemperatureAbNormalCount: false,
        RunStopCount: false,
        value: '_最近一周_',
        value1: '_里程统计报表_',
        timevalue1: '_最近一周_',
    ...
    filters: {
      translate: function (value) {
        if (value == "FHaveDataCount") {
          ...
          return "_超速报警报表_"
        } else if (value == "TemperatureAbNormalCount") {
          return "_温度异常统计_"
    ...
    methods: {
      ...
      updateHomeStyle: function () {
        ...
        var datajson = JSON.stringify({
          ...
          FT_ColdChain_HomePageLastStyle: this.FStatusBlock,
          FVersion: "1.0.0"
        });
        $.ajax({
      ...
      getHomeLastStyle: function () {
        ...
        $.ajax({
          ...
          "dataType": "json",
          "contentType": "application/json; charset=utf-8",
          "data": '{"FTokenID":"' + _this.$store.state.FTokenID + '","FAction":"QueryHomePageLastStyle","FSystemId":3,"FVersion":"1.0.0"}',
          success: function (json) {
            var obj = json;
...
//            obj.FObject[0].FOffLineCount=obj.FObject[0].FHaveDataCount-obj.FObject[0].FOnLineCount;
            ...
            //温度是否正常比例
            var sum = obj.FObject[0].FTemperatureNormalCount + obj.FObject[0].FTemperatureAbnormalCount;
            _this.FTemperatureNormalCount1=_this.deepCopy(_this.pieData);
            _this.FTemperatureNormalCount1.title.text="_温度是否正常比例_";
            _this.FTemperatureNormalCount1.series = [{
              type: 'pie',
              name: '_比例_',
              data: [
                {name: '_温度正常_',y: obj.FObject[0].FTemperatureNormalCount / sum * 100, color: '#035682'},
                {name: '_温度异常_',y: obj.FObject[0].FTemperatureAbnormalCount / sum * 100, color: '#498ca6'}
              ]
            }];
...
//				    UTCEndTime=this.parseUTC(endTime);
        $.ajax({
          ...
          success: function (data) {
            ...
            if (reportType == 'MileageCount') {
            ...
            } else if (reportType == 'TemperatureAbNormalCount') {
              _this.run.series[0].name = '_温度异常次数_';
              _this.run.yAxis.title.text = '_次数_(_次_)';
              _this.run.tooltip.formatter=function () {
                return '<b>' +this.x+ '</b><br/>' + this.series.name + ': ' + Highcharts.numberFormat(this.y, 0, '.', "") +'_次_' +'<br/>';
              };
              $.each(Fobj.FObject, function (index, obj) {
                var i=_this.run.xAxis.categories.indexOf(obj.FDate);
                if(i!=-1){
                  _this.run.series[0].data[i]=obj.FTemperatureAbNormalCount
                }
              });
              for (let i=0;i<_this.run.xAxis.categories.length;i++){
                if(_this.run.series[0].data[i]==undefined){
