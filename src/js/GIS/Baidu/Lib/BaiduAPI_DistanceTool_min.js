 ...
 * 主入口类是<a href="symbols/BMapLibDis.DistanceTool.html">DistanceTool</a>，
 * 基于Baidu Map API 1.2。
 *
...
/**
 * @namespace BMap的所有library类均放在BMapLibDis命名空间下
 */
var BMapLibDis = window.BMapLibDis = BMapLibDis || {};
...
(function() {
	...
	/**
	 * @exports DistanceTool as BMapLibDis.DistanceTool
	 */
	var DistanceTool =
	/**
	 * DistanceTool类的构造函数
	 ...
	 *
	 * @example <b>参考示例：</b><br />
	 * var map = new BMap.Map("container");<br />map.centerAndZoom(new BMap.Point(116.404, 39.915), 15);<br />var myDistanceToolObject = new BMapLibDis.DistanceTool(map, {lineStroke : 2});
	 */
		BMapLibDis.DistanceTool = function(map, opts){
			if (!map) {
				return;
			}
			 ...
			 */
			this._styles = {
				"BMapLibDis_diso" : "height:17px;width:5px;position:absolute;background:url(http://api.map.baidu.com/images/dis_box_01.gif) no-repeat left top"
				,"BMapLibDis_disi" : "color:#7a7a7a;position:absolute;left:5px;padding:0 4px 1px 0;line-height:17px;background:url(http://api.map.baidu.com/images/dis_box_01.gif) no-repeat right top"
				,"BMapLibDis_disBoxDis" : "color:#ff6319;font-weight:bold"
			};
	...
	DistanceTool.prototype.open = function(){
		...
		// 已有其他地图上的鼠标操作工具开启
		if (!!BMapLibDis._toolInUse) {
			return;
		}
		this._isOpen = true;
		BMapLibDis._toolInUse = true;
	...
	DistanceTool.prototype._formatSegLabel = function(label, text){
		...
		label.setContent("<span style='" + this._styles.BMapLibDis_diso + "'><span style='" + this._styles.BMapLibDis_disi + "'>" + text + "</span></span>");
	};
	...
	DistanceTool.prototype._formatTitle = function(type, text, distance, label){
		...
		if (type == 1){
			...
			htmls.push("<span>总长：<span style='" + this._styles.BMapLibDis_disBoxDis+"'>" + dis + "</span>" + unit + "</span><br />");
			...
			htmls.push("总长：<span style='" + this._styles.BMapLibDis_disBoxDis + "'>" + dis + "</span>" + unit);
		} else {
			title.setOffset(0, 25);
			htmls.push(t);
		}
