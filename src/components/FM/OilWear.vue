	...
	<section id="OilWear">
		...
		<div class="el_table_wrap" id="el_table_wrap"><!--part2 header start-->
			...
			<el-table :data="FuelCountInfo" stripe style="width: 100%" :height="table_height"
				...
				<el-table-column label="_操作_" align="center" width="80">
					<template scope="scope">
						<el-button type="text" size="small" @click="doChartQuery(scope.row)"><i
							class='img_chart'></i></el-button>
					</template>
				</el-table-column>
				<el-table-column prop="FShortName" label="_公司简称_" width="160">
				</el-table-column>
				<el-table-column prop="FVehicleName" label="_车牌号_" sortable min-width="160">
				</el-table-column>
				<el-table-column prop="FRefuelCount" label="_加油次数_" sortable width="120">
					<template scope="scope">
				...
				<el-table-column prop="FRefuelSum" label="_加油_" sortable width="120">
				</el-table-column>
				<el-table-column prop="FImproperCount" label="_异常消耗次数_" sortable width="180">
					<template scope="scope">
				...
				<el-table-column prop="FImproperSum" align="center" sortable label="_异常消耗_" width="120">
				</el-table-column>
				<el-table-column prop="FRunFuelSum" align="center" label="_运行消耗_" sortable width="120">
				</el-table-column>
				<el-table-column prop="FRunMileageSum" align="center" label="_运行距离_(km)" sortable width="180">
				</el-table-column>
				<el-table-column prop="MPG" align="center" label="_百公里油耗_(_升_/100km)" sortable width="220">
				</el-table-column>
			...
			<!--加油次数-->
			<el-table :data="FuelRefuelInfo" stripe style="width: 100%" :height="table_height" v-show="show_FuelRefuel">
				<!--表格统一用饿了么组件，类型使用stripe，不要使用border-->
				<el-table-column tooltip-effect="dark" type="index" align="center" label="_序号_" width="80">
				</el-table-column>
				<el-table-column label="_操作_" align="center" width="120">
					<template scope="scope">
						...
						<el-button type="text" size="small" @click="doRefuelQuery(scope.row,'refuel')">
							<i class='img_chart'></i>
						</el-button>
					</template>
				</el-table-column>
			...
			<!--异常次数-->
			<el-table :data="FuelImproperInfo" stripe style="width: 100%" :height="table_height"
				...
				<el-table-column label="_操作_" align="center" width="120">
					<template scope="scope">
						...
						<el-button type="text" size="small" @click="doRefuelQuery(scope.row,'improper')">
							<i class='img_chart'></i>
						</el-button>
					</template>
				</el-table-column>
				...
				<el-table-column prop="FImproper" align="center" label="_油位消耗_" sortable width="120">
				</el-table-column>
				<el-table-column prop="address" align="center" label="_位置_" min-width="230">
				</el-table-column>
			</el-table>
	...
	export default {
		data() {
			return {
				...
				activeName: "1",
				show_FuelCount: true,
				show_FuelRefuel: false,
				show_FuelImproper: false,
				table_height: null,
				div_height: null,
				FuelCountInfo: [],
				FuelRefuelInfo: [],
				...
				RefuelOption:{
					...
					xAxis: {
						...
						plotLines: [{
							...
							value: null,                //定义在哪个值上显示标示线，这里是在x轴上刻度为3的值处垂直化一条线
							width: 1                 //标示线的宽度，2px
						},
						{
							color: 'red',            //线的颜色，定义为红色
		...
		methods: {
			...
			doQuery(){
				...
				if(!this.datelimit(endTime,startTime,3)){
					this.warningDialogMsg("_查询时间请小于三天_!", "warning");
					return false;
				}
			...
			resetCondition(){
				...
				this.pageSize_FuelCount = 10;
				this.pageSize_Refuel = 10;
				this.pageSize_Improper = 10;
			...
			doChartQuery(row){
				...
				let _this = this;
				_this.option.xAxis.categories.length=0;
				_this.option.navigator.xAxis.categories.length=0;
				_this.option.series[0].data.length=0;
				_this.option.series[1].data.length=0;
				_this.option.series[2].data.length=0;
				_this.option.navigator.series.data.length=0;
				//_this.option.navigator.series[1].data.length=0;
				//_this.option.navigator.series[2].data.length=0;
				let time = this.seachTime.split(" - ");
				let startTime = this.parseUTC(time[0]+" 00:00:00");
				let endTime = this.parseUTC(time[1]+" 23:59:59");
				let letFTokenID = this.$store.state.FTokenID;
			...
			addFifteenMinute(type,time){
				if (type=="b"){
					let timeStr=this.GetDateSeconds(time)-(60*10*1);
				...
				$.post("web/FleetManager/FleetManager_LBS",{
					...
					if(data.Result=="200"){
						if(data.FObject.length!=0){
							...
							//console.log(data);
							if(type=="refuel"){
								_this.RefuelOption.xAxis.categories.length=0;
								//_this.RefuelOption.xAxis.plotLines[0].value=row.FStartDateTime.slice(-8);
								//_this.RefuelOption.xAxis.plotLines[1].value=row.FStopDateTime.slice(-8);
								_this.RefuelOption.series.length=0;
								if(data.FObject.length>0){
									if(data.FObject.FFuelValue1!=-1){
										_this.RefuelOption.series.push({
											name: '_油位_',
											data:[]
										});
									}
									/*if(data.FObject.Table1.FFuelValue2!=-1){
									 _this.RefuelOption.series.push({
									 name: '_油位2_',
									 data:[]
									 });
									 }
									 if(data.FObject.Table1.FFuelValue3!=-1){
									 _this.RefuelOption.series.push({
									 name: '_油位3_',
									 data:[]
									 });
								...
								for (let i = 0; i < _this.RefuelOption.xAxis.categories.length; i++) {
									let obj = _this.RefuelOption.xAxis.categories[i];
									//console.log(obj+","+row.FStartDateTime+","+i);
									if(obj==row.FStartDateTime){
										_this.RefuelOption.xAxis.plotLines[0].value=i;
									}
									if(obj==row.FStopDateTime){
										_this.RefuelOption.xAxis.plotLines[1].value=i;
									}
								}
								_this.showChartRefuelDialog=true;
								_this.showChartRefuel();
							}else if(type=="improper"){
								_this.ImproperOption.xAxis.categories.length=0;
								_this.ImproperOption.series.length=0;
								if(data.FObject.length>0){
									if(data.FObject.FFuelValue1!=-1){
										_this.ImproperOption.series.push({
											name: '_油位_',
											data:[]
										});
									}
								...
								for (let i = 0; i < _this.ImproperOption.xAxis.categories.length; i++) {
									let obj = _this.ImproperOption.xAxis.categories[i];
									if(obj==row.FStartDateTime){
										_this.ImproperOption.xAxis.plotLines[0].value=i;
									}
									if(obj==row.FStopDateTime){
										_this.ImproperOption.xAxis.plotLines[1].value=i;
									}
								}
								_this.showChartImproperDialog=true;
								_this.showChartImproper();
							}
						}else {
							_this.warningDialogMsg("_暂无数据_!", "warning");
						}
			...
			getUsableData(arr,type){
				...
				for (let i = 0; i < result.length; i++) {
					if(type=="countChart"){
						//result[i].FGPSDateTime=result[i].FGPSDateTime.replace(/T/g, " ");
						result[i].FGPSDateTime = this.parseGMT(result[i].FGPSDateTime);
						this.option.xAxis.categories.push(result[i].FGPSDateTime);
						this.option.navigator.xAxis.categories.push(result[i].FGPSDateTime);
						this.option.series[0].data.push(result[i].FSpeed);
						this.option.series[1].data.push(result[i].FMileage);
						//let FFuelValue=((result[i].FFuelValue1/(result[i].FFuelMax-result[i].FFuelMin))*result[i].FFuelSize).toFixed(2);
						this.option.series[2].data.push(Number(result[i].FFuelValue1));
						this.option.navigator.series.data.push(Number(result[i].FFuelValue1));
					}else if(type=="count"){
					...
					}else if(type=="refuelchart"){
						result[i].FGPSDateTime = this.parseGMT(result[i].FGPSDateTime);
						this.RefuelOption.xAxis.categories.push(result[i].FGPSDateTime);
						//let RefuelFuelValue=((result[i].FFuelValue1/(result[i].FFuelMax-result[i].FFuelMin))*result[i].FFuelSize).toFixed(2);
						if(result[i].FFuelValue1!=-1){
							this.RefuelOption.series[0].data.push(Number(result[i].FFuelValue1));
						}
						/*if(result[i].FFuelValue2!=-1){
							this.RefuelOption.series[1].data.push(Number(result[i].FFuelValue2));
						}
						if(result[i].FFuelValue3!=-1){
							this.RefuelOption.series[2].data.push(Number(result[i].FFuelValue3));
						}*/
						//this.RefuelOption.series[0].data.push(Number(result[i].FFuelValue1));
					}else if(type=="improperchart"){
						result[i].FGPSDateTime = this.parseGMT(result[i].FGPSDateTime);
						this.ImproperOption.xAxis.categories.push(result[i].FGPSDateTime);
						//let ImproperFuelValue=((result[i].FFuelValue1/(result[i].FFuelMax-result[i].FFuelMin))*result[i].FFuelSize).toFixed(2);
						if(result[i].FFuelValue1!=-1){
							this.ImproperOption.series[0].data.push(Number(result[i].FFuelValue1));
						}
						/*if(result[i].FFuelValue2!=-1){
							this.ImproperOption.series[1].data.push(Number(result[i].FFuelValue2));
						}
						if(result[i].FFuelValue3!=-1){
							this.ImproperOption.series[2].data.push(Number(result[i].FFuelValue3));
						}*/
						//this.ImproperOption.series[0].data.push(Number(result[i].FFuelValue1));
					...
					if (result[i].FLatitude != "" || result[i].FLongitude != undefined) {
						let lat = result[i].FLongitude;
						let lon = result[i].FLatitude;
						window.setTimeout(function(){
